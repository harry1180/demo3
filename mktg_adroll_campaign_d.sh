job_name='mktg_adroll_campaign_d'
...
config_file_name=${dwh_scripts_base_dir}/mktg_adroll_campaign_d/pythonscripts/config.json
...
adroll_to_json()
{
echo "calling adroll API and writing results as json file"
python ${dwh_scripts_base_dir}/mktg_adroll_campaign_d/pythonscripts/adroll_to_json.py -c ${config_file_name} -i ${Linux_Input} -o ${Linux_Input} -s ${date_load_from} -e ${date_load_to} --adroll_user=${adroll_user} --adroll_pswd=${adroll_pswd} --adroll_apikey=${adroll_apikey}
...
delete_if_exist_file="mktg_adroll_campaign_s.json"
sleep 2
...
copy_query="copy "$stage_db"."$stage_table_nm" from '"$s3_bucket_name"/"$job_name"/input/"${source_json_file_nm}"' credentials '$s3_prod_load_creds' JSON as '"$s3_bucket_name"/json/"$job_name"/"$jsonpaths_file_nm".jsonpaths' maxerror as 1 ACCEPTINVCHARS COMPUPDATE ON TRUNCATECOLUMNS BLANKSASNULL TIMEFORMAT AS 'YYYY-MM-DD HH:MI:SS';"
...
find ${Linux_Input} -name \*adroll*.json -exec rm {} \; || true
list_job_related_files
echo_processing_step ${job_name} "Removing Data Files" "Completed"

echo_processing_step ${job_name} "Download Adroll" "Started"
adroll_to_json
echo_processing_step ${job_name} "Download Adroll" "Completed"

echo_processing_step ${job_name} "uploading all the files to S3 and stage table" "Started"
cd ${Linux_Input}
#ls ${Linux_Input}*adroll*
#gzip -f *adroll*.json
ls ${Linux_Input}*.json | sort | uniq > ${Linux_Output}/${job_name}_upload_files.list
#ls ${Linux_Input}*adroll*.gz | sort | uniq > ${Linux_Output}/${job_name}_upload_files.list
...
arr["mktg_adroll_campaign_s"]="mktg_adroll_campaign_s.json"
for key in ${!arr[@]}; do
    echo ${key} ${arr[${key}]}
...
echo "step1_refresh_mktg_adroll_campaign_w.sql
step2_mktg_adroll_campaign_d_delete_expr.sql
step3_mktg_adroll_campaign_d_insert_existing.sql" | while read sql_script other_var
do
...
echo "mktg_adroll_campaign_d_dq.json" | while read script other_var
do
